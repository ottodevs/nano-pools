...
export function TransactionFeedback({ transactionHash, onClose }: TransactionFeedbackProps) {
  const { transactions, addTransaction, removeTransaction } = useTransactionMonitor(defaultChainId);
            ...
            <div className="space-y-1">
              {merits.slice(-3).map((merit, index) => (
                <div key={index} className="flex justify-between text-xs">
                  <span className="capitalize">{merit.type.replace('_', ' ')}</span>
...
export function useTransactionFeedback() {
  const [currentTransaction, setCurrentTransaction] = useState<string | undefined>();
