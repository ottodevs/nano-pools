...
export function useBlockscout(chainId: number) {
    ...
    async (
      ...
      try {
          ...
          status:
            transactionData.status === "1"
              ...
              : transactionData.status === "0"
                ? "failed"
                : "pending",
          blockNumber: transactionData.block_number
            ? parseInt(transactionData.block_number)
            : undefined,
        ...
        if (blockscoutTx.status === "success") {
          ...
          console.log(
            `🏆 Merit Reward Awarded: ${meritReward.amount} points for ${type}`,
            {
              transactionHash,
              poolId,
              amount,
        ...
        setError(
          err instanceof Error ? err.message : "Failed to monitor transaction"
        );
        throw err;
      } finally {
        setLoading(false);
  ...
  // Get merits by type
  const getMeritsByType = useCallback(
    (type: TransactionType) => {
...
export function useTransactionMonitor(chainId: number) {
  const [transactions, setTransactions] = useState<
    Map<string, BlockscoutTransaction>
  >(new Map());
